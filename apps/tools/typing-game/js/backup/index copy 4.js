...
function printWords() {
  ...
  // Add a data attribute to track if this word is removed by matching
  $w.data('word-id', id);
  $w.data('removed', false);
  
  // Store the word text in a data attribute for reference
  $w.data('word-text', text);
  ...
  $w.animate(
    { top: bottom },
    {
      ...
      complete: function () {
        ...
        // Check if this word was marked as removed
...
function matchWords() {
  ...
  if ($target.length) {
    // correct
    // Mark the word as removed BEFORE stopping the animation
    console.log("Word matched! Removing target word.");
    $target.data('removed', true);
    // Now stop all animations on the target and remove it
