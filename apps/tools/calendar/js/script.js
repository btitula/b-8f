...
//     // thisDay.classList.add("updated");
//     thisDay.classList.add("updated");
//     thisDay.innerHTML = `<p>${name}</p><p>${email}</p><p>${phone}</p>`;
//   });
...
bookingForm.addEventListener("submit", (e) => {
  ...
  const cbFoo = document.getElementById("cb-foo");
  const cbBar = document.getElementById("cb-bar");
  const cbFooChecked = cbFoo.checked;
  const cbBarChecked = cbBar.checked;
  ...
  selectedDatesArray.forEach((date) => {
    ...
    // cbFooChecked ? thisDay.innerHTML += `<p>Foo</p>` : null;
    // cbBarChecked ? thisDay.innerHTML += `<p>Bar</p>` : null;
    ...
    thisDay.innerHTML = `
      <ul class="mt-1 flex items-center justify-end [&>li:not(:first-child)]:-ml-2">
        <li class="px-2 py-2 h-8 rounded-full flex items-center justify-center text-sm font-medium bg-gray-300 ring-2 ring-white hover:z-10">
          ${dateString}
        </li>
        <li class="px-2 py-2 h-8 rounded-full flex items-center justify-center text-sm font-medium bg-[${getRandomColor()}] ring-2 ring-white hover:z-10">
          ${cbFooChecked ? "Foo Foo Foo" : ""}
        </li>
        <li class="px-2 py-2 h-8 rounded-full flex items-center justify-center text-sm font-medium bg-[${getRandomColor()}] ring-2 ring-white hover:z-10">
          ${cbBarChecked ? "Bar" : ""}
        </li>
