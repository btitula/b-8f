 ...
 */
function highlightKeyword(text: string, keyword: string): { html: string; count: number } {
...
function highlightText(): void {
  const textArea = document.getElementById('eo-text') as HTMLTextAreaElement;
  const keywordInput = document.getElementById('eo-keyword') as HTMLInputElement;
...
function checkPassword() {
  const passwordInput = document.getElementById('et-password') as HTMLInputElement;
  ...
  if (minLength && upperCase && lowerCase && digit && specialChar) {
    ...
    message += '<ul>';
    if (!minLength) message += '<li>Độ dài >= 8</li>';
    if (!upperCase) message += '<li>Ít nhất 2 chữ HOA</li>';
    if (!lowerCase) message += '<li>Ít nhất 2 chữ thường</li>';
    if (!digit) message += '<li>Ít nhất 1 số</li>';
    if (!specialChar) message += '<li>Ít nhất 1 ký tự đặc biệt (!@#$%^&*())</li>';
    message += '</ul>';
  }
  // console.log(message)
...
let numbers: number[] = [5, 2, 1, 9, 8, 0]
...
function insertIntoSortedArray(arr: number[], newNum: number): number[] {
  arr.push(newNum);
